...
class RepoInstaller:
    ...
    def _ensure_user_level_shims(self) -> None:
        """Create the ``~/.agent/`` shim symlinks needed by hardcoded paths.

        Several components of the system embed absolute paths to
        ``~/.agent/shared-repo-memory/...`` and ``~/.agent/skills/<skill>/...``
        rather than resolving their install root at runtime:

          - Git hooks generated by bootstrap-repo.py reference
            ``$HOME/.agent/shared-repo-memory/<script>.py`` (pre-commit,
            build-catchup).
          - Skill SKILL.md instructions tell Claude to invoke
            ``$HOME/.agent/shared-repo-memory/<script>.py`` and
            ``~/.agent/skills/<skill>/...`` helper scripts.
          - Helper shell scripts (promote-adr.sh, notify-wrapper.sh) hardcode
            the same paths.

        A full per-repo refactor would push the install root through every
        such call site. Until that lands, a single symlink at
        ``~/.agent/shared-repo-memory/`` plus per-skill symlinks under
        ``~/.agent/skills/`` make the existing call sites resolve correctly
        without copying any state. The shim is shared across all per-repo
        installs on this machine and survives ``uninstall --repo``; it is
        only removed by a global uninstall.
        """
        shim_root: Path = Path.home() / ".agent"

        # Helper-scripts shim: ~/.agent/shared-repo-memory -> source scripts/.
        helper_shim: Path = shim_root / "shared-repo-memory"
        if self.dry_run:
            log(f"[DRY-RUN] would ensure {helper_shim} -> {self.scripts_root}")
        else:
            shim_root.mkdir(parents=True, exist_ok=True)
            if helper_shim.is_symlink():
                if helper_shim.resolve() == self.scripts_root.resolve():
                    pass  # Already correct.
                else:
                    helper_shim.unlink()
                    helper_shim.symlink_to(self.scripts_root)
                    log(f"updated shim {helper_shim} -> {self.scripts_root}")
            elif helper_shim.exists():
                log(
                    f"skipping {helper_shim} shim: target exists as a real "
                    "directory (likely from a prior --global install); leaving "
                    "it untouched"
                )
            else:
                helper_shim.symlink_to(self.scripts_root)
                log(f"created shim {helper_shim} -> {self.scripts_root}")

        # Skills shim: ~/.agent/skills/<name> -> wherever .claude/skills/<name>
        # already resolves to. Mirrors what the user installed via ai-skills
        # (or this installer's own skills/ dir). Lets SKILL.md instructions
        # that say ``~/.agent/skills/<name>/<helper>`` find their helpers.
        skills_shim_dir: Path = shim_root / "skills"
        local_skills_dir: Path = self.target_repo_root / ".claude" / "skills"
        if local_skills_dir.is_dir():
            if not self.dry_run:
                skills_shim_dir.mkdir(parents=True, exist_ok=True)
            for skill_link in sorted(local_skills_dir.iterdir()):
                if not skill_link.is_symlink():
                    continue
                try:
                    skill_target: Path = skill_link.resolve(strict=True)
                except (OSError, RuntimeError):
                    continue
                shim_skill: Path = skills_shim_dir / skill_link.name
                if self.dry_run:
                    log(f"[DRY-RUN] would ensure {shim_skill} -> {skill_target}")
                    continue
                if shim_skill.is_symlink():
                    if shim_skill.resolve() == skill_target:
                        continue
                    shim_skill.unlink()
                elif shim_skill.exists():
                    # Real directory present; do not clobber.
                    continue
                shim_skill.symlink_to(skill_target)
                log(f"created shim {shim_skill} -> {skill_target}")
